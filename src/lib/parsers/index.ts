...
import { isSyntheticDigestSession, isRepoMonitorCommitSession } from './synthetic-sessions';
...
export async function scanAllSessions(
  ...
  for (const session of results.flat()) {
    if (isSyntheticDigestSession(session)) continue; // self-generated by daily-digest, not real work
    if (isRepoMonitorCommitSession(session)) continue; // RepoMonitor auto-push commit-message helper, not real work
